...
function PopupWithForm({title, onSwitchPopup, textSwitch, submitButtonText, isOpen, onClose, onSubmit, children,  onReqError, registerPopup, registerNameValid}) {
  ...
  return (
    <div className={isOpen ? 'popup popup_opened': 'popup'} onClick={handleClickClosePopup} onKeyDown={handleKeyClosePopup}>
      <form className="popup__container" onSubmit={handleSubmit} noValidate>
        ...
        <input className="popup__input" type="email" name="email" value={emailValue} placeholder="Введите почту" onChange={handleEmailChange} disabled={submitDisable ? false : true} required></input>
        ...
        <input className="popup__input" type="password" name="password" value={passwordValue} onChange={handlePasswordChange} placeholder="Введите пароль" disabled={submitDisable ? false : true} required></input>
        ...
        <button type="submit" className="popup__submit-button" disabled={(submitDisable)&&(emailValid)&&(passwordValid)&&(registerPopup ? registerNameValid : true) ? false : true}>{submitButtonText}</button>
